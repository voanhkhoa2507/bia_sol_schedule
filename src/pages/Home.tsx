...
export default function Home() {
  ...
  const [schedules, setSchedules] = useState<Profile[]>([]);
  ...
  const handleRemove = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (window.confirm("Bạn có chắc muốn xóa lịch trình này? Các môn học bên trong cũng sẽ bị mất.")) {
      try {
        await deleteDoc(doc(db, 'profiles', id));
        // Note: For a complete app, we'd also delete all schedule items matching this personId
      } catch (error) {
  ...
  const handleAdd = async () => {
    ...
    if (name) {
      try {
        const colors = ['bg-emerald-500', 'bg-blue-500', 'bg-indigo-500', 'bg-rose-500', 'bg-amber-500'];
    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-slate-50">
      <div className="max-w-3xl w-full mx-auto space-y-12">
        <header className="text-center space-y-4">
          ...
          >
            Chọn thời khóa biểu để xem hoặc chỉnh sửa. Dữ liệu được đồng bộ qua Firebase.
          </motion.p>
        </header>
